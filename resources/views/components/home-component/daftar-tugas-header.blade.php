...
<div class="hstack gap-2">
    ...
    <div class="p-2">
        <a href="{{ route('employment-detail.show', $data->employmentDetail) }}" class="text-decoration-none text-dark">
        ...
        @if ($data->id === auth()->id())
            <form action="{{ route('switchUnit') }}" method="POST">
                @csrf
                <select name="unit_id" onchange="this.form.submit()" class="form-select form-select-sm border-0"
        ...
        <div class="small text-muted">
            @if ($data->latestLogin)
                <small class="text-muted">
                    Login terakhir:
                    {{ $data->latestLogin->login_at->diffForHumans() }}
                </small>
            @else
